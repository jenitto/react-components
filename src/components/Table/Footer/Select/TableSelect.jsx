...
const TableSelect = ({ selected, options, disabled, onSelectionChange }) => {
	...
	return (
		<div
			ref={ref}
			className={`sftk-table-select ${opened ? 'opened' : 'closed'} ${disabled ? 'disabled' : ''}`}>
			<div
				className={`sftk-table-select__selected ${disabled ? 'disabled' : ''}`}
				tabIndex={`${disabled ? -1 : 0}`}
				onKeyDown={handleKeyDownSelect}
				onClick={() => handleOpenSelect(!opened)}
			>
				{selected}
				<svg className="sftk-table-select__select-icon" width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
					<mask id="mask5" mask-type="alpha" maskUnits="userSpaceOnUse" x="5" y="9" width="14" height="8">
						<path d="M6.27148 9.39041L5.55273 10.1092L11.8027 16.3592L12.1621 16.7029L12.5215 16.3592L18.7715 10.1092L18.0527 9.39041L12.1621 15.281L6.27148 9.39041Z" fill="#858585" />
					</mask>
					<g mask="url(#mask5)">
			...
			{
				opened
					? <div className="sftk-table-select__select">
						<div
							className="sftk-table-select__focus-trap"
							tabIndex="0"
						...
						{options.map((item, index) => (
							<div
								...
								className={`sftk-table-select__option ${item.disabled ? 'disabled' : ''} ${isSelected(item) ? 'selected' : ''}`}
								tabIndex="0"
								onClick={() => handleSelectChanges(item)}
								onKeyDown={(e) => handleKeyDownOption(e, item, index)}>
								<span className="sftk-table-select__option-label">{item}</span>
							</div>
						))}
						<div
							className="sftk-table-select__focus-trap"
